...
let testCode = curry(add)
console.log(testCode(1)(2)(3))
console.log(testCode(1, 2)(3))
console.log(testCode(1)(2, 3))
```
...
//这主要是因为网上很多实现方法没有针对 array 做处理，直接将其当成 object
//这样拷贝后虽然值没问题，但是 array 的元素会被转化为 object。这显然是错误的做法。
```
...
> 方法二：reduce + concat + isArray + recursivity
...
> 方法三：forEach+isArray+push+recursivity
